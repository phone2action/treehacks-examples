...
// Get the Phone2Action access token by texting "TREEHACKS" to 52886
const PHONE2ACTION_ACCESS_TOKEN = process.env.PHONE2ACTION_ACCESS_TOKEN
  ? process.env.PHONE2ACTION_ACCESS_TOKEN
  : config.get("phone2actionAccessToken");
...
function receivedMessage(event) {
  ...
  if (messageText) {
    ...
    switch (
      ...
      default:
        sendLegislatorsMessage(senderID, messageText);
    }
...
/**
 * Use Phone2Action API to lookup legislators
 */
function sendLegislatorsMessage(recipientId, messageText) {
  request(
    {
      uri:
        "https://q4ktfaysw3.execute-api.us-east-1.amazonaws.com/treehacks/legislators",
      qs: { address: messageText, level: "NATIONAL_UPPER" },
      headers: {
        "x-api-key": PHONE2ACTION_ACCESS_TOKEN
      },
      method: "GET"
    },
    function(error, response, body) {
      if (!error && response.statusCode == 200) {
        ...
        JSON.parse(body).officials.forEach(official => {
          officialsNames.push(`${official.first_name} ${official.last_name}`);
        });
        ...
        const messageData = {
          ...
          message: {
            text: `Your U.S. Senators are: ${officialsNames.join(", ")}`,
            metadata: "DEVELOPER_DEFINED_METADATA"
          }
        };
        callSendAPI(messageData);
        ...
        const messageData = {
          ...
          message: {
            text: "Couldn't find your officials, sorry.",
            metadata: "DEVELOPER_DEFINED_METADATA"
          }
        };
        callSendAPI(messageData);
